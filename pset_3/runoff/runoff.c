...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            preferences[voter][rank] = i; // Recording voting preference of each candidate
            return true;
        }
    }
    return false;
...
bool is_tie(int min)
{
    // Is everyone's vote count equal to minimal vote count? If yes, then it's a tie
    bool is_everything_min = true;

    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes != min && !candidates[i].eliminated)
        {
            is_everything_min = false;
        }
    }

    return is_everything_min;
}
